import { call, put, take, takeEvery, fork, select, delay, takeLatest } from 'redux-saga/effects';
import {add, complete, get, remove, turnCompletedAll, unCompletedAll, update, deleteAllTodoCompleted } from "../actions";
...
function* doHandleUpdate(action) {
    ...
    yield axios.put(`/todo/${itemEdit.id}`, {...itemEdit, text: value}).catch((err) => {
...
function* watchRemoveCompletedAllCallAPI(){
    yield takeEvery(TYPE_ACTION.TODO.UNCOMPLETED_ALL_SAGA, doRemoveCompletedAllAPI)
}
...
function* watchTurnCompletedAllCallAPI() {
    yield takeLatest(TYPE_ACTION.TODO.TURN_COMPLETED_ALL_SAGA, doTurnCompletedAllAPI)
}
function* watchRemoveAllToDoCompleted(){
    yield takeEvery(TYPE_ACTION.TODO.DELETE_ALL_TODO_COMPLETED_SAGA, doRemoveAllToDoCompleted)
}
export {
    watchGetTodosSaga,
    watchAddTodoSaga,
